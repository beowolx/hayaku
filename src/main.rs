...
fn parse_to_raw(data: &[u8], start: usize, end: usize) -> u32 {
  let raw =
    u32::from_be_bytes(unsafe { *data.get_unchecked(start..).as_ptr().cast() });
  raw >> (8 * (4 - (end - start)))
}
...
fn raw_to_pdep(raw: u32) -> u64 {
  ...
  #[cfg(not(feature = "no_pdep"))]
  {
    ...
    let pdep =
      0b0000000000000000001111000000000000011111111000001111111111111111u64;
...
fn iter_lines<'a>(
  ...
  let find = |last: usize, sep: S| {
    let simd =
      S::from_array(unsafe { *data.get_unchecked(last..).as_ptr().cast() });
